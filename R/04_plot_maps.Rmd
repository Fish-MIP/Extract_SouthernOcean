...
names(phyt_raster)<-"index_1" # WARNING aes(fill = index_1) for modelled data and aes(fill = layer) fo empirical data - possibly due to raster calcualtions, need to check code 
# use dec-mar data 
phyto_emp_plot<-plot_SO(wmap_df = antmap_df, data_to_plot = phyt_raster, proj = circumpolarCRS, legend_name = legend_name, min = min, max = max, data_type = "phyto")
phyto_emp_plot

names(phyt_raster2)<-"index_1"
# use all summer data
phyto_emp_plot2<-plot_SO(wmap_df = antmap_df, data_to_plot = phyt_raster2, proj = circumpolarCRS, legend_name = legend_name, min = min, max = max, data_type = "phyto")
phyto_emp_plot2
...
# zooplankton  
min = 0
max = 25 # 25 # as per Yang et al. 2022
legend_name = "g C m-2"

names(zoop_raster)<-"index_1"
zoo_emp_plot<-plot_SO(wmap_df = antmap_df, data_to_plot = zoop_raster, proj = circumpolarCRS, legend_name = legend_name, min = min, max = max, data_type = "zoo")
zoo_emp_plot
...
zoo_plot<-plot_SO(wmap_df = antmap_df, data_to_plot = mean_zoo, proj = circumpolarCRS, legend_name = legend_name, min = mean_zoo@data@min, max = mean_zoo@data@max, data_type = "modelled") # almost empty high number so adjustng the range
