...
Dowloaded on 30/04/2023 - Phyto and zooplnkton are downloaded from DKRZ as per ISIMIP3b protocl: phyc and zooc (total concentration - https://fish-mip.github.io/protocols.html), vertically integrated. 

We consider ISIMIP3b data as we have MEM outputs linked to these inputs for ISIMIP3b and not 3a yet. 
...
Check if there is 3d data for both phyc and zooc - yes both variables are available in 3D and vertically integrated

### *TO CHECK* 

File orientation (upside-down?) and check same things as for 01 - OK

Strange time dimension now adjusted but this is my interpretation. Asked Matthias, interpretation is OK - starting value is Jan 1850.  
...
For better comparison use **zmeso-vint**? zmeso-vint: Integrated Mesozooplankton/Large Zoo Carbon Concentration. Discussed with Ryan too - not fully understanding this output -e.g. which size included and how this has been created - would use total zooplankton instead and specify in figs.  
...
phyto_ant<-mclapply(netcdf_phyto, function(x) extract_antarctica_inputs(x, history_year1 =history_year1, history_year2=history_year2,future_year1 = future_year1,future_year2=future_year2), mc.cores = detectCores()-2)

names(phyto_ant) <- netcdf_phyto

zoo_ant<-mclapply(netcdf_zoo, function(x) extract_antarctica_inputs(x, history_year1 =history_year1, history_year2=history_year2,future_year1 = future_year1,future_year2=future_year2), mc.cores = detectCores()-2)
...
if(removeNA == "narmT"){
  save(mean_phyto, mean_zoo, file = "/home/ubuntu/Extract_SouthernOcean/Tmp_data/02_extract_input_SO_means.RData")
}else if(removeNA == "narmF"){
