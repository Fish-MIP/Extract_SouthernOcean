...
mean_phyto<-stackApply(file_phyto, indices =  rep(1,nlayers(file_phyto)), fun = "mean", na.rm = T)

file_zoo<-stack(zoo_ant[[1]], zoo_ant[[2]])
mean_zoo<-stackApply(file_zoo, indices =  rep(1,nlayers(file_zoo)), fun = "mean", na.rm = T)

# CHECK 
# calculate mean using other approach for size class 2 and compare results to above approach 

trial<-list(phyto_ant$`gfdl-esm4_r1i1p1f1_historical_phyc-vint_60arcmin_global_monthly_1850_2014.nc`, phyto_ant$`ipsl-cm6a-lr_r1i1p1f1_historical_phyc-vint_60arcmin_global_monthly_1850_2014.nc`)
...
#### WARNING - very different from before - is this becasue of changes in units?? ALOS CONSIDER that now you are looking at a DIFFERENT SPATIAL COVERAGE OF DATA (-60 before, -35 now)
# check files and means 
plot(mean_phyto)
plot(mean_zoo)
...
save(mean_phyto, mean_zoo, file = "/home/ubuntu/Extract_SouthernOcean/Tmp_data/02_extract_input_SO_means2.RData")
